              ...
              <div className="inner">
                ...
                <div className="block-content">
                  <div className={classNames('grid', {'grid-col-2': _.get(section, 'col_number', null) === 'two', 'grid-col-3': _.get(section, 'col_number', null) === 'three'})}>
                    {_.map(_.get(this.props, 'data.doc_sections.sections', null), (doc_section, doc_section_idx) => {
                        let doc_section_path = pathJoin(_.get(this.props, 'data.doc_sections.root_docs_path', null), doc_section);
                        let doc_section_page = getPage(this.props.pages, doc_section_path);
                        return (
                          <div key={doc_section_idx} className="grid-item">
                            <div className="grid-item-inside">
                              <h3 className="grid-item-title line-left"><Link href={withPrefix(_.get(doc_section_page, '__metadata.urlPath', null))}>{_.get(doc_section_page, 'frontmatter.title', null)}</Link></h3>
                              ...
                              <div className="grid-item-buttons">
                                <Link href={withPrefix(_.get(doc_section_page, '__metadata.urlPath', null))}>Learn More</Link>
                              </div>
                            </div>
                          </div>
                        )
